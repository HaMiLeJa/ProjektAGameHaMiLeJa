...
public class CollectableManager : MonoBehaviour
{
    ...
    //  MIRO SCHREIBT:
    //wenn du willst das 3 stück auf einmal spawnen sollen, kannst du entweder eine verkettete liste nutzen, oder ein Array oder ein event trigger.
    ...
    void SetCollectableReferencesAtStart(Hex hex)
    {
        ...
        {
            hex.SpawnCollectable(); Debug.Log("GElse");
        }
    }
    void Update()
    {
        ...
        */
        // Debug.Log(AllCollectables.Count.ToString());
#if UNITY_EDITOR
        if (Input.GetKeyDown(KeyCode.J)) OnRespawnCollectables?.Invoke();
        if (Input.GetKeyDown(KeyCode.O)) Debug.Log(AllCollectables.Count);
#endif
    }
    public void StartCollectableSpawn()
    {
        foreach (KeyValuePair <GameObject, CollectableReferences> hex in AllCollectables)
        {   //&& if Distance to player ist höher als was auch immer
            if(!hex.Value.ActiveCollectable && DistanceToPlayer(hex.Key) >= 100f)hex.Value.HexScript.SpawnCollectable();
        }
    }
    float DistanceToPlayer(GameObject hex)
    {
        Vector3 Verbindungsvector = ReferenceLibrary.Player.transform.position - hex.transform.position;
