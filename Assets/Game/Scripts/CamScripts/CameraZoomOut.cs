 ...
 void Update() 
    {
        if(!PortalManager.CameraTeleportActive)
     {
        ...
        lerpedValueMoonX = MathLibary.RemapClamped( ZoomingValue.x, ZoomingValue.y, cashedXScaleMoon, cashedXScaleMoon+ addXScaleMoon, xzVelocity);
            lerpedValueMoonZ = MathLibary.RemapClamped( ZoomingValue.x, ZoomingValue.y, cashedZScaleMoon, cashedZScaleMoon+ addZScaleMoon, xzVelocity);
            lerpedValueGhostLayerX = MathLibary.RemapClamped( ZoomingValue.x, ZoomingValue.y, cashedXScaleGhostLayer, cashedXScaleGhostLayer+ addXScaleGhostLayer, xzVelocity);
            lerpedValueGhostLayerZ = MathLibary.RemapClamped( ZoomingValue.x, ZoomingValue.y, cashedZScaleGhostLayer, cashedZScaleGhostLayer+ addZScaleGhostLayer, xzVelocity);
            if (cashedFov + ZoomOutDelay < lerpedValue)
       {
           vcam.m_Lens.FieldOfView = Mathf.Lerp(vcam.m_Lens.FieldOfView, lerpedValue, zoomOutRoughness*Time.deltaTime);
        ...
        shakeAngle = Random.Range(1,9);
            ...
            if(_cameraShakeCollision.camShakeActivated && ! deactivateShaking && xzVelocity > StartShakingAtVelocity && nextShakeAllowed && !GameManager.bridgePause)
            {
