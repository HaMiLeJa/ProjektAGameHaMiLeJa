...
public class Portal : MonoBehaviour //Portal in zwei Richtungen, Frei untereinander verlinkbar
{
    ...
    private float distanceBetweenCamHelperAndPlayerCashed, cashedFovTemp, cashedFovNewCam,
                   cashedVelocity, cashedlerpValue, cashedXVelocity, cashedZVelocity, xzVelocity;
    ...
    private void FixedUpdate()
    {
        if (GameManager.CameraTeleportActive)
        {
           ...
           if( distanceCamHelperPlayer > distanceBetweenCamHelperAndPlayerCashed/2)
            ...
            //Zero out speed 
           if(distanceCamHelperPlayer > Mathf.Abs(8)) _playerMovement.rb.velocity = Vector3.zero;
           ...
           if (distanceCamHelperPlayer < Mathf.Abs(8))
           {
               ...
               _playerMovement.rb.velocity = new Vector3(
                   horizontalInput*(_gameManager.SpeedAfterTeleport + (cashedVelocity/_gameManager.ReduceSpeedInfluenceBeforeTeleport
                                                                       *_gameManager.IncreaseSpeedInfluenceBeforeTeleport)+forceWhenNothingIsPressed/5f),
                   0,
                   verticalInput*(_gameManager.SpeedAfterTeleport+ (cashedVelocity/_gameManager.ReduceSpeedInfluenceBeforeTeleport
                                                                    *_gameManager.IncreaseSpeedInfluenceBeforeTeleport)+forceWhenNothingIsPressed/5f)
                   );
               ...
               GameManager.CameraTeleportActive = false; SetBackFollowSpeed();
           }
    ...
    {
        if (other.tag == player.tag && GoalPortal)
        {
